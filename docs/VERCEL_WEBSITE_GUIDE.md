...
**Suggested:** **Next.js 14+ (App Router)** in a folder `website/` in the same repo (monorepo) or a **separate GitHub repo** linked to Vercel.
...
Create `website/.env.local` locally (copy from example). On Vercel: **Project → Settings → Environment Variables**.
...
Example `website/.env.local.example`:
...
### A. Push `website/` to GitHub

Commit the `website` folder (not `.env.local`).

### B. Connect Vercel

1. Go to [vercel.com](https://vercel.com) → sign in with GitHub.  
2. **Add New Project** → import your repository.  
3. **Root Directory:** set to `website` (important if monorepo).  
...
| 2 | Create `website/` with Next.js landing pages | ☐ |
| 3 | Deploy to Vercel (root dir = `website`) | ☐ |
...
Only to **develop** (`npm run dev` in `website/`). Production runs on Vercel’s build servers.
